...
def as_pages(plugin, func, next=0, per_request=0, *args, **kwargs):
...
class Collection(_TAXIIEndpoint):
    ...
    def get_objects(self, plugin=None, accept=MEDIA_TYPE_TAXII_V21, **filter_kwargs):
        """Implement the ``Get Objects`` endpoint (section 5.3)"""
        self._verify_can_read()
        query_params = _filter_kwargs_to_query_params(filter_kwargs)
        if plugin and plugin.logger:
            plugin.logger.debug(
                f"{plugin.log_prefix}: Request details - URL: {self.objects_url}, params: {query_params}, headers.accept: {accept}"
            )
        return self._conn.get(self.objects_url, headers={"Accept": accept}, params=query_params, plugin=plugin)
