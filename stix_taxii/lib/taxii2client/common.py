...
def _filter_kwargs_to_query_params(filter_kwargs):
    ...
    for kwarg, arglist in six.iteritems(filter_kwargs):
        ...
        if not hasattr(arglist, "__iter__") or isinstance(
            arglist, six.string_types
        ):
            arglist = (arglist,)
        ...
        if kwarg == "version":
            ...
            if len(arglist) > 1:
                raise InvalidArgumentsError(
                    "No more than one value for filter"
...
def _grab_total_items(resp):
    ...
    try:
        ...
        return int(results.group(2)) - int(results.group(1)) + 1, int(
            results.group(3)
        )
    except ValueError as e:
        six.raise_from(
...
class _TAXIIEndpoint(object):
    ...
    def __init__(
        ...
        """
        if (conn and ((user or password) or auth)) or (
            (user or password) and auth
        ):
            raise InvalidArgumentsError(
                "Only one of a connection, username/password, or auth object may"
                " be provided."
            )
        elif conn:
            self._conn = conn
...
class _HTTPConnection(object):
    ...
    def valid_content_type(self, content_type, accept):
        ...
        if self.version == "2.0":
            return all(
                ...
                content_type_tokens[0] == "application/vnd.oasis.taxii+json"
                or content_type_tokens[0] == "application/vnd.oasis.stix+json"
            )
        else:
            return (
                all(elem in content_type_tokens for elem in accept_tokens)
                and content_type_tokens[0] == "application/taxii+json"
            )

    def get(self, url, headers=None, params=None, plugin=None):
    ...
    def post(self, url, headers=None, params=None, **kwargs):
        ...
        if len(kwargs) > 1:
            raise InvalidArgumentsError(
                "Too many extra args ({} > 1)".format(len(kwargs))
            )
