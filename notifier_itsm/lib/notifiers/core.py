...
class SchemaResource(ABC):
    ...
    def _get_environs(self, prefix: str = None) -> dict:
        ...
        return dict_from_environs(
            prefix, self.name, list(self.arguments.keys())
        )
    ...
    def _validate_data(self, data: dict):
        ...
        if e:
            ...
            raise BadArguments(
                validation_error=msg, provider=self.name, data=data
            )
    ...
    def notify(
        ...
        **kwargs,
        ...
        :raises: :class:`~notifiers.exceptions.NotificationError` if ``raise_on_errors`` is set to True and response
         contained errors
        """
        logger.info(f"{log_prefix}: Calling process data with kwargs {kwargs}")
        data = self._process_data(**kwargs)
        logger.info(f"{log_prefix}: Processed data: {data}")
        logger.info(f"{log_prefix}: Calling send notification method.")
        rsp = self._send_notification(data)
        logger.info(
            f"{log_prefix}: Notification successfully sent. Response: {rsp}"
        )
        if raise_on_errors:
            logger.info(f"{log_prefix}: Unable to send notification.")
            rsp.raise_on_errors()
        return rsp
...
def notify(provider_name: str, **kwargs) -> Response:
    ...
    """
    return get_notifier(provider_name=provider_name, strict=True).notify(
        **kwargs
    )
