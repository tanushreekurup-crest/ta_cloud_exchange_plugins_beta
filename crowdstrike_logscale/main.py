...
class CrowdStrikeLogScalePlugin(PluginBase):
    ...
    def _convert_timestamp(self, events):
        ...
        for event in events:
            if event:
                epoch_timestamp = event.get("x-cs-timestamp") or event.get("timestamp")
    ...
    def transform(self, raw_data, data_type, subtype) -> List:
        ...
        if not self.configuration.get("transformData", True):
            try:
                ...
                self.logger.error(
                    message="{}: {} Error: {}".format(self.log_prefix, err_msg, err),
                    details=traceback.format_exc(),
                )
                raise CrowdStrikeLogScaleException(err_msg)
            except MappingValidationError as err:
                err_msg = "Validation error occurred for {} mapping file.".format(
                    PLATFORM_NAME
                )
                self.logger.error(
                ...
                err_msg = (
                    "An error occurred while mapping data using given json mappings."
                )
                self.logger.error(
                    message="{}: {} Error: {}".format(self.log_prefix, err_msg, err),
                    details=traceback.format_exc(),
    ...
    def push(self, transformed_data, data_type, subtype):
        ...
        try:
            crowdstrike_logscale_client.push(transformed_data, data_type, subtype, uid)
        except Exception as err:
            ...
            err_msg = f"Could not ingest data into CrowdStrike LogScale. UUID: {uid}"
            self.logger.error(
                message="{}: {} Error: {}".format(self.log_prefix, err_msg, err),
                details=traceback.format_exc(),
            )
    ...
    def validate_auth(self, configuration: dict) -> ValidationResult:
        ...
        if response.status_code == 200:
            ...
            err_msg = (
                "Make sure provided token is ingest token with valid permission(s)."
            )
            self.logger.error(
                message=f"{self.log_prefix}: {err_msg}",
                details=f"Received API response: {response.text}",
            ...
            self.logger.error(
                message=(
                    ...
                    "response code {}.".format(self.log_prefix, response.status_code)
                ),
                details=f"Received API response: {response.text}",
            )
    ...
    def validate(self, configuration: dict) -> ValidationResult:
        ...
        if type(
            mappings
        ) != dict or not crowdstrike_logscale_validator.validate_mappings(mappings):
            self.logger.error(
            ...
            return ValidationResult(
                success=False,
                message="Invalid {} attribute mapping provided.".format(PLATFORM_NAME),
            )
